...
import { Card, CardContent, CardActions, Typography, Button, Dialog,
...
export function RoomCard({ room, myRole }: { room: Room; myRole: 'admin' | 'user' }) {
  ...
  const created = useMemo(() => {
    try {
      return room.createdAt?.toDate?.().toLocaleString() ?? '';
    } catch {
      return '';
    }
  }, [room.createdAt]);
  ...
  return (
    <Card>
      <CardContent>
        ...
        {room.description && <Typography sx={{ mt: 1 }}>{room.description}</Typography>}
        {created && (
